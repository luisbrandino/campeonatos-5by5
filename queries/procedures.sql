...
exec buscar_time_com_maior_pontuacao 1
...
exec buscar_time_com_mais_gols 1
...
exec buscar_maior_numero_de_gols_de_cada_time 1
...
exec buscar_jogo_com_mais_gols 1
...
exec criar_time "Caribe", "Piratas", '2015-05-01'
...
exec criar_campeonato 'Liga Europa'
exec definir_campeao_campeonato 1
...
select * from tb_jogo
exec criar_jogo 1, 2, 1;
exec definir_gols_time_mandante 1, 2, 1, 333;
exec definir_gols_time_visitante 1, 2, 1, 906;

create or alter proc participante_existe @campeonato_id int, @time_id int
as
	if exists(select 1 from tb_campeonato where campeonato_id = @campeonato_id and time_id = @time_id)
		select 1
	else
		select 0
	return
...
exec criar_participante 1, 1
exec criar_participante 1, 2

exec adicionar_pontos 1, 1, -13;
exec adicionar_pontos 1, 2, -5;
exec adicionar_gols 1, 2, -18;
